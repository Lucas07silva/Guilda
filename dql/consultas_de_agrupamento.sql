...
SELECT desenvolvedor, COUNT(id_jogo) AS total_jogos
FROM jogos
...
SELECT id_perfil, COUNT(id_conquista) AS conquistas_desbloqueadas
FROM conquistas_usuarios
GROUP BY id_perfil;

-- 5. Quantidade de tópicos criados em cada discussão de fórum
SELECT id_discussao, COUNT(id_topico) AS total_topicos
FROM topicos_forum
...
SELECT regiao, COUNT(id_usuario) AS total_usuarios
FROM usuarios
GROUP BY regiao;
