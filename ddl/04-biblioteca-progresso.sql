...
-- views
CREATE OR REPLACE VIEW biblioteca_usuario AS
SELECT 
    b.id_usuario,
    b.id_jogo,
    ...
    (
        ...
        JOIN conquista c ON cu.id_conquista = c.id
        WHERE cu.id_usuario = b.id_usuario AND c.id_jogo = b.id_jogo
    ) AS conquistas_desbloqueadas
FROM biblioteca b;

-- trigger function
CREATE OR REPLACE FUNCTION atualizar_tempo_biblioteca()
RETURNS TRIGGER AS $$
DECLARE
    v_minutos_jogados INT;
BEGIN
    v_minutos_jogados := EXTRACT(EPOCH FROM (NEW.data_fim - NEW.data_inicio)) / 60;

    UPDATE biblioteca 
    SET tempo_jogado_min = COALESCE(tempo_jogado_min, 0) + v_minutos_jogados
...
-- triggers
CREATE TRIGGER tg_atualizar_tempo_sessao
AFTER UPDATE OF data_fim ON historico_sessao
FOR EACH ROW
WHEN (OLD.data_fim IS NULL AND NEW.data_fim IS NOT NULL)
EXECUTE FUNCTION atualizar_tempo_biblioteca();
