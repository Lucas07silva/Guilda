...
-- Adiciona colunas para armazenar as métricas de avaliação na tabela jogo
ALTER TABLE jogo ADD COLUMN IF NOT EXISTS total_avaliacoes INT DEFAULT 0;
ALTER TABLE jogo ADD COLUMN IF NOT EXISTS total_positivas INT DEFAULT 0;
ALTER TABLE jogo ADD COLUMN IF NOT EXISTS percentual_aprovacao DECIMAL(5,2) DEFAULT 0.00;


-- 2. PROCEDURE: RegistrarAvaliacao
-- Facilita a inserção de novas avaliações e garante o sequenciamento do ID
...
DECLARE
    v_novo_id INT;
BEGIN
    -- verifica o último ID inserido na tabela e soma 1
    SELECT COALESCE(MAX(id), 0) + 1 INTO v_novo_id FROM avaliacao;

    -- insere o registro na tabela de avaliações
    INSERT INTO avaliacao (id, id_usuario, id_jogo, texto, recomendado_bool, data_postagem)
    VALUES (v_novo_id, p_id_usuario, p_id_jogo, p_texto, p_recomendado, CURRENT_DATE);

    RAISE NOTICE 'Avaliação registrada com sucesso! Novo ID: %', v_novo_id;
END;
$$;


-- TRIGGER E FUNÇÃO: AtualizarMediaRating 
-- recalcula as médias do jogo automaticamente após qualquer alteração nas avaliações

-- criação da função que será executada pela Trigger
CREATE OR REPLACE FUNCTION fn_AtualizarMediaRating()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    -- define qual jogo precisa ser atualizado (OLD para deletes, NEW para inserts/updates)
    IF (TG_OP = 'DELETE') THEN
        v_id_jogo := OLD.id_jogo;
    ELSE
        v_id_jogo := NEW.id_jogo;
    END IF;

    -- conta o total de avaliações e quantas foram recomendadas (TRUE)
    SELECT 
        COUNT(*), 
        COALESCE(SUM(CASE WHEN recomendado_bool = TRUE THEN 1 ELSE 0 END), 0)
    ...
    IF v_total > 0 THEN
        v_percentual := (v_positivas::DECIMAL / v_total::DECIMAL) * 100.00;
    ELSE
        v_percentual := 0.00;
    END IF;

    -- atualiza os dados consolidados na tabela jogo
    UPDATE jogo
    SET total_avaliacoes = v_total,
        total_positivas = v_positivas,
        percentual_aprovacao = v_percentual
    WHERE id = v_id_jogo;

    RETURN NULL; -- retorna nulo pois é uma trigger disparada AFTER (depois) da ação
END;
$$;

-- criação da Trigger vinculada à tabela avaliacao
DROP TRIGGER IF EXISTS AtualizarMediaRating ON avaliacao;

CREATE TRIGGER AtualizarMediaRating
AFTER INSERT OR UPDATE OR DELETE ON avaliacao
FOR EACH ROW
EXECUTE FUNCTION fn_AtualizarMediaRating();

-- VIEW JogosEmDestaque
-- vitrine de jogos classificada pela aprovação da comunidade

CREATE OR REPLACE VIEW JogosEmDestaque AS
SELECT 
    j.id AS id_jogo,
    j.titulo,
    ...
    j.preco_base,
    j.total_avaliacoes,
    j.percentual_aprovacao,
    CASE 
        WHEN j.total_avaliacoes = 0 THEN 'Sem avaliações'
        WHEN j.percentual_aprovacao >= 90 THEN 'Extremamente Positivas'
        WHEN j.percentual_aprovacao >= 75 THEN 'Muito Positivas'
        WHEN j.percentual_aprovacao >= 50 THEN 'Mistas'
        ELSE 'Negativas'
    END AS status_comunidade
FROM jogo j
JOIN publicadora p ON j.id_publicadora = p.id
WHERE j.total_avaliacoes > 0
ORDER BY j.percentual_aprovacao DESC, j.total_avaliacoes DESC;
